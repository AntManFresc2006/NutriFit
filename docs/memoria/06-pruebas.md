...
### RegistroEjercicioServiceImpl — 14 tests

Este servicio coordina tres colaboradores: `RegistroEjercicioRepository`, `EjercicioRepository` y `PerfilRepository`. Los tests se organizan en cuatro clases anidadas: `CalcularKcal`, `Registrar`, `FindByUsuarioAndFecha` y `DeleteById`.

**Verificación de la fórmula MET.** La clase `CalcularKcal` contiene cuatro tests que verifican el método estático `calcularKcal(met, pesoKg, duracionMin)`. La fórmula aplicada es `MET × pesoKg × (duracionMin / 60.0)`, con redondeo a dos decimales mediante `HALF_UP`. Los valores esperados se calculan analíticamente antes de escribir cada test:

```
correr (MET=8.0), 75 kg, 45 min → 8.0 × 75 × 0.75 = 450.00 kcal
caminar (MET=3.5), 80 kg, 30 min → 3.5 × 80 × 0.50 = 140.00 kcal
ciclo con decimales (MET=5.0), 70 kg, 20 min → 5.0 × 70 × 0.333... = 116.67 kcal
```

El cuarto test verifica específicamente el redondeo: `116.6666...` debe producir `116.67`, no `116.66`.

**Validación fail-fast en el registro.** Los tests de `Registrar` comprueban con `verify(..., never())` que las operaciones de persistencia no se invocan cuando la validación previa falla. Si el ejercicio no existe, el servicio no consulta el perfil ni llama a `save`. Si el usuario no tiene perfil, `save` tampoco se invoca:
...
**Origen del nombre del ejercicio.** Un test verifica que `nombreEjercicio` en la respuesta proviene de la entidad recuperada del repositorio, no de ningún campo del request. Esto garantiza que el cliente no puede inyectar un nombre arbitrario.

**Pertenencia del registro en el borrado.** El test `registroDeOtroUsuario_lanzaExcepcionConAmbosIdsYSinBorrar` verifica que el mensaje de error incluye tanto el id del registro como el del usuario, facilitando el diagnóstico, y que `deleteById` no llega a invocarse:
...
`BaseApiClientTest` accede al método protegido `validarRespuesta` mediante una subclase stub interna, evitando exponer el método en la API pública:
...
La métrica relevante es la cobertura de los servicios probados, que son la única capa donde reside la lógica que puede fallar de forma silenciosa:

| Servicio                    | Cobertura de líneas |
|-----------------------------|---------------------|
| `AuthServiceImpl`           | 100 %               |
| `ComidaServiceImpl`         | 100 %               |
| `PerfilServiceImpl`         | 100 %               |
